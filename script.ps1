...
"
    ...
    if ($variable -eq 1) {
        ...
        write-host "LOG : L'utilisateur $name a bien ete ajoute"
    } elseif ($variable -eq 2) {
        write-host "Utilisateurs existants : $users"
        $rm = read-host "Quel est le login de l'utilisateur a supprimer ?"
        remove-localuser $rm
        write-host "LOG : L'utilisateur possedant le login $rm a bien ete supprime"
    } elseif ($variable -eq 3) {
        write-host "Groupes existants : $groups"
        $groupname = read-host "Quel est le nom du groupe a creer ?"
